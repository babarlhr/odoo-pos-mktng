...
(function() {
  ...
  DivSugar.Matrix = (function() {
    ...
    Matrix.prototype.setQuaternion = function(q) {
        var m = new DivSugar.Matrix().fromQuaternion(q);
        var v = new DivSugar.Vector();
        this.getScale(v);
    ...
    Matrix.prototype.getRotation = function() {
        var q = new DivSugar.Quaternion().fromMatrix(this);
        return q.toEuler();
    };
    ...
    Matrix.prototype.getPosition = function(vec) {
        vec.set(this.trans);
        return this;
    };
    ...
    Matrix.prototype.getScale = function(vec) {
      vec.x = this.xAxis.norm();
      vec.y = this.yAxis.norm();
      vec.z = this.zAxis.norm();
      return this;
    };

    Matrix.prototype.setScale = function(x, y, z) {
      this.xAxis = this.xAxis.normalize().mul(x);
      this.yAxis = this.yAxis.normalize().mul(y);
      this.zAxis = this.zAxis.normalize().mul(z);
      return this;
    };
  ...
  DivSugar.Quaternion = (function() {
    ...
    Quaternion.prototype.toEuler = function() {
        var q = {x: this.w, y:this.x, z:this.y, w:this.z};
        return [
            DivSugar.RAD_TO_DEG * Math.atan2(2*(q.x*q.y + q.z*q.w), 1 - 2*(q.y*q.y + q.z*q.z)),
            DivSugar.RAD_TO_DEG * Math.asin(2*(q.x*q.z - q.w*q.y)),
            DivSugar.RAD_TO_DEG * Math.atan2(2*(q.x*q.w + q.y*q.z), 1 - 2*(q.z*q.z + q.w*q.w))
        ];
    };

    Quaternion.prototype.fromEuler = function(x,y,z) {
        x *= DivSugar.DEG_TO_RAD;
        y *= DivSugar.DEG_TO_RAD;
        z *= DivSugar.DEG_TO_RAD;

        var cx = Math.cos(x/2), sx = Math.sin(x/2);
        var cy = Math.cos(y/2), sy = Math.sin(y/2);
        var cz = Math.cos(z/2), sz = Math.sin(z/2);
  ...
  DivSugar.Scene = (function() {
    ...
    Scene.prototype.setImage = function(src, callback) {
      ...
      if (src == null) {
        ...
        this.div.style.backgroundImage = 'none';
      } else if (src.indexOf('linear-gradient') === 0) {
          this.div.style.backgroundColor = 'transparent';
          this.div.style.background = src;
      } else {
        this.div.style.backgroundColor = 'transparent';
    ...
    Scene.prototype.setRadius = function(rtl,rtr,rbl,rbr) {
        if (arguments.length === 1) {
            this.div.style['border-radius'] = ''+rtl+'px';
        } else {
            this.div.style['border-radius'] = '' + (rtl || 0) + 'px '
                                            + '' + (rtr || 0) + 'px '
                                            + '' + (rbl || 0) + 'px '
                                            + '' + (rbr || 0) + 'px';
        }
  ...
  DivSugar.Node = (function() {
    ...
    Node.prototype.getPosition = function(vec) {
      this._transform.getPosition(vec);
      return this;
    };
    ...
    Node.prototype.getScale = function(vec) {
        if (vec) {
            this._transform.getScale(vec);
            return this;
        } else {
            vec = new DivSugar.Vector();
            this._transform.getScale(vec);
            return vec;
        }
    };
    ...
    Node.prototype.setQuaternion = function(q) {
        this._transform.setQuaternion(q);
        this.div.style[DivSugar._cssTransform] = this._transform._toCSSTransform();
        return this;
    };
    ...
    Node.prototype.setRadius = DivSugar.Scene.prototype.setRadius;
            ...
            for (param in _ref3) {
              ...
              switch (param) {
                ...
                case 'position':
                  ...
                  break;
                case 'rotation':
                  var q = new DivSugar.Quaternion(animTask._fromRotation).slerp(animTask._toRotation,a1);
                  this._lastRotation = q;
